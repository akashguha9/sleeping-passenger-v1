...
// ---------------------------------------------------------------------------
// Watchdog status panel — surfaces the contents of
// runtime/refresh_watchdog_summary.json so the operator can see whether
// the 30-minute self-healing watchdog ran, what it found, and what
// improved.  Advisory-only: the panel never triggers a refresh, never
// authorises a trade, and clearly states so in the footer.
// ---------------------------------------------------------------------------

function watchdogChipForStatus(status?: string): { chip: string; color: string; label: string } {
  switch ((status ?? 'UNKNOWN').toUpperCase()) {
    case 'HEALTHY':
      return { chip: 'sp-chip sp-chip-cyan', color: 'var(--sp-cyan)', label: 'HEALTHY' };
    case 'IMPROVED_BUT_STALE':
      return { chip: 'sp-chip sp-chip-warn', color: 'var(--sp-gold)', label: 'IMPROVED · STILL STALE' };
    ...
    case 'MISSING':
      return { chip: 'sp-chip sp-chip-warn', color: 'var(--sp-gold)', label: 'MISSING · NEVER RAN' };
    default:
      return { chip: 'sp-chip', color: 'var(--sp-mist)', label: status ?? 'UNKNOWN' };
  }
}

function WatchdogStatusPanel({ summary }: { summary: WatchdogSummaryResponse | null }) {
  ...
  const trackedRows: { key: string; label: string; before: any; after: any }[] = [
    {
      key: 'kalshi',
      label: 'Kalshi',
    ...
    >
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {summary.present && summaryStale && (
          <span
            ...
            data-testid="watchdog-summary-stale-chip"
          >
            Summary stale · last write &gt; {summary.summary_stale_after_minutes ?? 60}m ago
          </span>
        )}
      </div>
      ...
      {summary.present ? (
        <>
          ...
          <div className="grid gap-1 md:grid-cols-2 text-xs" style={{ color: 'var(--sp-mist)' }}>
            ...
            <div data-testid="watchdog-stale-after">
              ...
              <span
                className="font-mono"
                style={{ color: staleAfter.length === 0 ? 'var(--sp-cyan)' : 'var(--sp-gold)' }}
              >
            ...
            <div data-testid="watchdog-dependency-critical">
              Dependency-critical:{' '}
              <span className="font-mono" style={{ color: 'var(--sp-bone)' }}>
                {dependencyCritical.length === 0 ? 'none' : dependencyCritical.join(', ')}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {trackedRows.map(({ key, label, before, after }) => {
              const fsAfter = after?.freshness_state ?? after?.present === false ? after?.freshness_state : after?.freshness_state;
            ...
            >
              ...
              {pmdStaleParents.length > 0 ? ` · parent stale: ${pmdStaleParents.join(', ')}` : ''}
            </span>
            <span
              className="sp-chip"
          ...
          <p className="text-[10px]" style={{ color: 'var(--sp-mist)' }} data-testid="watchdog-jitter-line">
            Jitter:{' '}
            <span className="font-mono" style={{ color: 'var(--sp-bone)' }}>
              {jitterEnabled
        ...
        <p className="text-xs" style={{ color: 'var(--sp-mist)' }} data-testid="watchdog-never-ran">
          {summary.reason ??
