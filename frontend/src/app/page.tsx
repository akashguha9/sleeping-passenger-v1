...
function SourceHealthCard({ data }: { data: SourceHealthResponse | null }) {
  if (!data) {
    return (
      <div className="bg-slate-800/40 border border-slate-700/40 rounded-lg p-5 text-center">
        <p className="text-xs text-slate-500">Source health unavailable — backend offline</p>
      </div>
    );
  }

  const stats = data.fabric_stats;
  const rows = [
    { label: 'Snapshot rows', value: stats.total_snapshot_rows },
    { label: 'Signal events', value: stats.total_signal_events },
    { label: 'Tickers observed', value: stats.total_tickers_observed },
  ];
    ...
    <div className="bg-slate-800/60 border border-slate-700/60 rounded-lg p-5">
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between gap-3">
          <span className="text-xs text-slate-400">Bull state</span>
          <span className="text-xs font-mono text-sky-400">{data.fabric_bull_state}</span>
        </div>
      </div>
      <div className="mt-3 pt-3 border-t border-slate-700/40 flex items-center justify-between text-xs text-slate-600">
        <span className="flex items-center gap-1">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" /> live
        </span>
        <span>{formatTs(data.generated_at)}</span>
      </div>
    </div>
  );
}
