...
def _seed_db(tmp_path: Path, rows: list[dict]) -> Path:
...
def test_empty_db_returns_no_windows(tmp_path: Path):
...
def test_three_observations_within_window_yield_one_window(tmp_path: Path):
    rows = [
    ...
    conn = _ro(_seed_db(tmp_path, rows))
    try:
...
def test_two_observations_do_not_form_window_with_min_count_three(tmp_path: Path):
    rows = [
...
def test_observations_outside_window_do_not_merge(tmp_path: Path):
...
def test_source_filter_is_respected(tmp_path: Path):
    rows = [
        {"ts": "2026-04-21T00:00:00Z", "source": "A", "payload_json": "{}"},
        {"ts": "2026-04-21T00:05:00Z", "source": "A", "payload_json": "{}"},
...
def test_since_until_bounds_are_inclusive(tmp_path: Path):
    rows = [
...
def test_unparseable_timestamp_is_skipped_not_fabricated(tmp_path: Path):
    rows = [
        {"ts": "2026-04-21T00:00:00Z", "source": "A", "payload_json": "{}"},
...
def test_mixed_source_without_filter_reports_mixed(tmp_path: Path):
    rows = [
        {"ts": "2026-04-21T00:00:00Z", "source": "A", "payload_json": "{}"},
...
def test_invalid_window_minutes_raises(tmp_path: Path):
    db_path = _seed_db(tmp_path, [])
    conn = _ro(db_path)
    try:
...
def test_open_readonly_missing_db_raises(tmp_path: Path):
    with pytest.raises(sqlite3.OperationalError):
        open_readonly(tmp_path / "does-not-exist.db")
...
def test_cli_reports_error_on_missing_db(tmp_path: Path):
...
def test_cli_on_seeded_db_returns_window(tmp_path: Path):
    rows = [
        {"ts": ts, "source": "snapshot_logger", "payload_json": "{}"}
        for ts in (
            "2026-04-21T00:00:00Z",
