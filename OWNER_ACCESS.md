...
```powershell
# 1. Generate the owner token into the gitignored .env
...
| Layer | Behavior |
| --- | --- |
| Startup preflight | No `MVP_API_TOKEN` → `StartupSecurityError`, server refuses to boot. `MVP_ALLOW_UNAUTH=1` is the only bypass and works **only on a loopback bind** — an unauthenticated non-loopback server is never bootable. |
...
| Streamlit dashboard | Refuses non-loopback bind unless `MVP_DASHBOARD_ALLOW_NONLOOPBACK=1`. Local-only by policy. |
...
`python scripts/generate_api_token.py --write-env` any time; restart the
backend; re-paste in the browser panel. Old token dies immediately.

## GitHub repository settings checklist (manual — verify yourself)

Claude Code cannot verify org/repo admin settings from this session.
