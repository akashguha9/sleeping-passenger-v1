...
def backfill(
    ...
    *,
    ...
    and listing each affected (trade_id, event_type) tuple.
    """
    ...
    for trade, rec in pairs:
        ...
        if not write:
            ...
            if ticker in TARGETED_TICKERS:
                report["targeted_ticker_status"][ticker] = "would_write"
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = backfill(db_path, write=args.write, only_tickers=only)
    if args.json:
